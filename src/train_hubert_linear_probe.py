# src/train_hubert_linear_probe.py
...
from models.hubert_head import HuBERTClassifierHead


def load_hubert_backbone(model_id: str, device: torch.device):
    """
    Loads HuBERT-ECG backbone from Hugging Face.
    Expects input shape (B, T) and returns last_hidden_state (B, L, H).
    """
...
def eval_window_level(backbone, head, dataloader, device):
    ...
    for xb, yb, _rids in tqdm(dataloader, desc="[Eval windows]"):
        ...
        x_bt = xb.squeeze(1)  # (B, 3000) as required by HF HuBERT-ECG
        out = backbone(x_bt)
...
def eval_record_level(backbone, head, dataloader, device, *, verbose=True):
    """
    Record-level evaluation by averaging window logits per record_id.
    Also returns record_stats for variance analysis.
    """
    head.eval()
...
def train():
    ...
    # Choose HuBERT-ECG checkpoint from HF
    model_id = "Edoardo-BS/hubert-ecg-small"

    project_root = Path(__file__).resolve().parents[1]
    models_dir = project_root / "models"
    models_dir.mkdir(parents=True, exist_ok=True)
    best_ckpt_path = models_dir / "hubert_linear_probe_best.pt"
    ...
    head = HuBERTClassifierHead(in_dim=512, num_classes=4, dropout=0.3).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        eval_window_level(backbone, head, val_dl, device)
    ...
    print("\nLoading best head for test evaluation...")
